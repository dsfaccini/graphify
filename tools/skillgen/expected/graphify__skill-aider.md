...
Path('.graphify_transcripts.json').write_text(json.dumps(media_to_transcript, ensure_ascii=False))
...
Path('.graphify_detect.json').write_text(json.dumps(detect, ensure_ascii=False))
...
_fresh_path = Path('.graphify_incremental_fresh.json')
...
_transcripts_path = Path('.graphify_transcripts.json')
...
"
rm -f .graphify_detect.json .graphify_extract.json .graphify_ast.json .graphify_semantic.json .graphify_analysis.json .graphify_labels.json .graphify_incremental.json .graphify_incremental_fresh.json .graphify_transcripts.json; find . -maxdepth 1 -name '.graphify_chunk_*.json' -delete 2>/dev/null
...
Path('.graphify_incremental_fresh.json').write_text(json.dumps(new_extraction, ensure_ascii=False))
...
Path('.graphify_extract.json').write_text(json.dumps(merged_out, ensure_ascii=False))
